CLASS zclmm_ce_inventory_log DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      ty_t_report_s TYPE SORTED TABLE OF zc_mm_ce_inventory_log
                    WITH NON-UNIQUE KEY DocumentId,
      ty_t_report   TYPE STANDARD TABLE OF zc_mm_ce_inventory_log,
      ty_t_return   TYPE STANDARD TABLE OF bapiret2,

      BEGIN OF ty_filter,
        DocumentId TYPE RANGE OF zc_mm_ce_inventory_log-DocumentId,
        Line       TYPE RANGE OF zc_mm_ce_inventory_log-Line,
        Msgid      TYPE RANGE OF zc_mm_ce_inventory_log-Msgid,
        Msgty      TYPE RANGE OF zc_mm_ce_inventory_log-Msgty,
        Msgno      TYPE RANGE OF zc_mm_ce_inventory_log-Msgno,
        Msgv1      TYPE RANGE OF zc_mm_ce_inventory_log-Msgv1,
        Msgv2      TYPE RANGE OF zc_mm_ce_inventory_log-Msgv2,
        Msgv3      TYPE RANGE OF zc_mm_ce_inventory_log-Msgv3,
        Msgv4      TYPE RANGE OF zc_mm_ce_inventory_log-Msgv4,
        Message    TYPE RANGE OF zc_mm_ce_inventory_log-Message,
      END OF ty_filter.
...
* ---------------------------------------------------------------------------
    ...
    TRY.
        es_filter-line              = VALUE #( FOR ls_range IN it_range[ name = 'LINE' ]-range (
                                                                         sign   = ls_range-sign
                                                                         option = ls_range-option
                                                                         low    = ls_range-low
                                                                         high   = ls_range-high ) ).
    ...
    TRY.
        es_filter-msgid             = VALUE #( FOR ls_range IN it_range[ name = 'MSGID' ]-range (
                                                                         sign   = ls_range-sign
                                                                         option = ls_range-option
                                                                         low    = ls_range-low
                                                                         high   = ls_range-high ) ).
    ...
    TRY.
        es_filter-msgty             = VALUE #( FOR ls_range IN it_range[ name = 'MSGTY' ]-range (
                                                                         sign   = ls_range-sign
                                                                         option = ls_range-option
                                                                         low    = ls_range-low
                                                                         high   = ls_range-high ) ).
    ...
    TRY.
        es_filter-msgno             = VALUE #( FOR ls_range IN it_range[ name = 'MSGNO' ]-range (
                                                                         sign   = ls_range-sign
                                                                         option = ls_range-option
                                                                         low    = ls_range-low
                                                                         high   = ls_range-high ) ).
    ...
    TRY.
        es_filter-msgv1             = VALUE #( FOR ls_range IN it_range[ name = 'MSGV1' ]-range (
                                                                         sign   = ls_range-sign
                                                                         option = ls_range-option
                                                                         low    = ls_range-low
                                                                         high   = ls_range-high ) ).
    ...
    TRY.
        es_filter-msgv2             = VALUE #( FOR ls_range IN it_range[ name = 'MSGV2' ]-range (
                                                                         sign   = ls_range-sign
                                                                         option = ls_range-option
                                                                         low    = ls_range-low
                                                                         high   = ls_range-high ) ).
    ...
    TRY.
        es_filter-msgv3             = VALUE #( FOR ls_range IN it_range[ name = 'MSGV3' ]-range (
                                                                         sign   = ls_range-sign
                                                                         option = ls_range-option
                                                                         low    = ls_range-low
                                                                         high   = ls_range-high ) ).
    ...
    TRY.
        es_filter-msgv4             = VALUE #( FOR ls_range IN it_range[ name = 'MSGV4' ]-range (
                                                                         sign   = ls_range-sign
                                                                         option = ls_range-option
                                                                         low    = ls_range-low
                                                                         high   = ls_range-high ) ).
    ...
    TRY.
        es_filter-message           = VALUE #( FOR ls_range IN it_range[ name = 'MESSAGE' ]-range (
                                                                         sign   = ls_range-sign
                                                                         option = ls_range-option
                                                                         low    = ls_range-low
                                                                         high   = ls_range-high ) ).
...
* Recupera os logs
* ----------------------------------------------------------------------
    SELECT * FROM zi_mm_ce_inventory_log
        WHERE DocumentId IN @ls_filter-DocumentId
          AND Line       IN @ls_filter-Line
          AND Msgid      IN @ls_filter-Msgid
          AND Msgty      IN @ls_filter-Msgty
          AND Msgno      IN @ls_filter-Msgno
          AND Msgv1      IN @ls_filter-Msgv1
          AND Msgv2      IN @ls_filter-Msgv2
