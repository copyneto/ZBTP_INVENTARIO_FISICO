CLASS zclmm_ce_vh_storage_location DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES: ty_t_data TYPE STANDARD TABLE OF zclmm_s41_cds_inventario=>tys_zi_mm_vh_storage_locationt,
           ty_cds    TYPE zi_mm_ce_vh_storage_location,
...
* ---------------------------------------------------------------------------
    TRY.
        ...
        DELETE lt_range WHERE name = 'PLANT'.
        DELETE lt_range WHERE name = 'PLANTNAME'.
        DELETE lt_range WHERE name = 'STORAGELOCATIONNAME'.
...
* ---------------------------------------------------------------------------
    ...
    lo_remote->call_odata_read_list( EXPORTING iv_cds_name = 'ZI_MM_VH_STORAGE_LOCATION'
                                               it_range    = lt_range
...
* ----------------------------------------------------------------------
  ...
  METHOD filter_search_expression.
    ...
    LOOP AT ct_data REFERENCE INTO DATA(ls_data).
      ...
      FIND lv_search IN ls_data->storagelocation.

      IF sy-subrc EQ 0.
        CONTINUE.
      ENDIF.

      FIND lv_search IN ls_data->storagelocationname.
