CLASS zclmm_ce_vh_material DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES: ty_t_data TYPE STANDARD TABLE OF zclmm_s41_cds_inventario=>tys_zi_mm_vh_materialtype,
           ty_cds    TYPE zi_mm_ce_vh_material,
...
* ---------------------------------------------------------------------------
    ...
    lo_remote->call_odata_read_list( EXPORTING iv_cds_name = 'ZI_MM_VH_MATERIAL'
                                               it_range    = lt_range
...
* ----------------------------------------------------------------------
  ...
  METHOD filter_search_expression.
    ...
    LOOP AT ct_data REFERENCE INTO DATA(ls_data).
      ...
      FIND lv_search IN ls_data->material.

      IF sy-subrc EQ 0.
        CONTINUE.
      ENDIF.

      FIND lv_search IN ls_data->materialname.
